...
def scrape_airport_weather(station, start_date, end_date):
    ...
    with open(outfile, mode) as f:
        ...
        while start_date != end_date:
            ...
            format_search_url = search_url.format(station,
                                                  start_date.year,
                                                  start_date.month,
                                                  start_date.day)
...
def scrape_airport_weather_day(url):
    station = re.findall(r".*[/].*[/].*[/].*[/](.*)[/].*[/]",url)
    year = re.findall(r".*[/].*[/].*[/].*[/].*[/].*[/].*[/]([^-]*)[-]",url)
    month = re.findall(r".*[/].*[/].*[/].*[/].*[/].*[/].*[/].*[-]([^-]*)[-]",url)
    day = re.findall(r".*[/].*[/].*[/].*[/].*[/].*[/].*[/].*[-].*[-](.*)",url)    
    # make a new directory for each airport that you scrape
    # data for, to keep files separate and organized
    if not os.path.exists(station[0]):
        os.mkdir(station[0]) 
        
    
    # csv file name that can be formatted for any airport and any year
    outfile = '{}/{}_{}.csv'.format(station[0], station[0], year[0])
    ...
    with open(outfile, mode) as f:
        ...
        # write the observation date into the file
        f.write('{}-{}-{},'.format(year[0], month[0], day[0]))
        # write just the temperature and precipitation data into the file
        f.write(','.join(row[:12]))
        # new line, in case you want to append more rows to the same file later on
        f.write('\n')
...
if __name__ == '__main__':
    ...
    end_date = datetime(year=2012, month=1, day=3)
